...
- [ ] Constructors take an options object: `ProvenanceMark.from({ … })`, `ProvenanceMarkGenerator.from({ res, seed })`, `next(date, { info })`, `identifier({ style, words, prefix })`, `formatReport(report, { format })` (§3–§5).
...
- [ ] `identifier({ words })` outside 4 to 32, an unknown `style`, an unknown `formatReport` format, an unknown resolution name: `RangeError` (§3, §5).
...
| `idBytewords(n, prefix)` / `idBytemoji(n, prefix)` / `idBytewordsMinimal(n, prefix)` | `identifier({ style: "bytewords" \| "bytemoji" \| "minimal", words: n, prefix })` (defaults: bytewords, 4, no prefix); `words` outside 4 to 32 or fractional, or an unknown `style`, is a `RangeError` |
| `bytewordsIdentifier(prefix)` / `bytemojiIdentifier(prefix)` / `bytewordsMinimalIdentifier(prefix)` | `identifier({ prefix })`, `identifier({ style: "bytemoji", prefix })`, `identifier({ style: "minimal", prefix })` |
| `identifier()` (8 hex characters) | `idHex.slice(0, 8)` |
| `disambiguatedIdBytewords(marks, prefix)` / `disambiguatedIdBytemoji(marks, prefix)` | `ProvenanceMark.disambiguatedIdentifiers(marks, { style, prefix })` |
| `precedesOpt(next)` | `checkPrecedes(next)` (throws `Validation`) |
| `toBytewords()` / `toBytewordsWithStyle(style)` | `toBytewords({ style })` |
...
| `toUrl(base: string)` (a `TypeError` on a bad base) | `toUrl(base: string \| URL)`; `Url` when it does not parse; an existing `provenance` parameter is replaced |
...
| `g.next(date, info)` | `g.next(date, { info })`; a date the resolution cannot encode (`YearOutOfRange`, `DateOutOfRange`, `InvalidDate`) throws before any state changes |
...
| Before | After |
|---|---|
| `serialize2Bytes`/`4`/`6`, `deserialize2Bytes`/`4`/`6`, `serializeDate(res, d)`, `deserializeDate(res, b)`, `SerializableDate` | `encodeDate(date, { resolution })` / `decodeDate(bytes, { resolution })`; an invalid `Date` is `InvalidDate`; the messages are the reference's (`invalid date length: expected 2, 4, or 6 bytes, got N`, `seconds value too large for u32`, …) |
| `serializeSeq(res, n)` / `deserializeSeq(res, b)` | `encodeSeq(n, { resolution })` / `decodeSeq(bytes, { resolution })` |
...
| `ValidationReportFormat.Text` / `.JsonCompact` / `.JsonPretty`; `formatReport(report, format)` | `formatReport(report, { format: "text" \| "jsonCompact" \| "jsonPretty" })` (default `"text"`); an unknown format is a `RangeError` |
...
[`BlockchainCommons/provenance-mark-ts`](https://github.com/BlockchainCommons/provenance-mark-ts).
