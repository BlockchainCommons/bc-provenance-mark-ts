 ...
 *
 * Usage:
 *   bun scripts/api-report.ts --local   # (re)generate api/provenance-mark.api.md
 ...
 * (api/provenance-mark.api.md) is the reviewable record of the public surface: every
...
try {
  ...
  if (result.apiReportChanged && !local) {
    console.error(
      "Public API surface changed but api/provenance-mark.api.md was not updated.\n" +
  ...
  console.log(
    local
      ? "API report (api/provenance-mark.api.md) is up to date."
