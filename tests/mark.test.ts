...
import { ProvenanceMark, ProvenanceMarkGenerator, ProvenanceMarkInfo } from "../src";
...
describe("ProvenanceMark", () => {
  describe("Low resolution", () => {
    it("should generate expected marks with passphrase 'Wolf'", () => {
      ...
      expect(marks.map((m) => m.idHex.slice(0, 8))).toEqual(expectedIds);
    ...
    it("should serialize and deserialize via bytewords", () => {
      ...
      const restored = ProvenanceMark.fromBytewords("low", bytewords);
    ...
    it("should serialize and deserialize via CBOR", () => {
      ...
      const cborData = mark.toCbor().toData();
  ...
  describe("JSON serialization", () => {
    it("should serialize and deserialize via JSON", () => {
      ...
      expect(mark.idHex.slice(0, 8)).toEqual(restored.idHex.slice(0, 8));
  ...
  describe("test_low (Rust parity)", () => {
    ...
    it("should match Rust expected bytewords", () => {
      ...
      for (let i = 0; i < marks.length; i++) {
        const restored = ProvenanceMark.fromBytewords("low", EXPECTED_BYTEWORDS_LOW[i]);
    ...
    it("should match Rust expected UR strings", () => {
      ...
      // Verify UR strings
      expect(marks.map((m) => m.toUR().toString())).toEqual(EXPECTED_URS_LOW);

      // Verify roundtrip
      for (let i = 0; i < marks.length; i++) {
        const restored = ProvenanceMark.fromUR(UR.parse(EXPECTED_URS_LOW[i]));
        expect(marks[i].equals(restored)).toBe(true);
      }
    });
  ...
  describe("Seed serialization (Rust parity)", () => {
    it("should serialize and deserialize seed via JSON", () => {
      ...
      expect(mark1.idHex.slice(0, 8)).toEqual(mark2.idHex.slice(0, 8));
    });
  });
