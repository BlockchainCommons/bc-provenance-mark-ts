...
function createTestMarks(
  ...
  const generator = ProvenanceMarkGenerator.fromPassphrase(resolution, passphrase);
...
describe("Validation (Rust parity)", () => {
  ...
  describe("test_validate_multiple_chains", () => {
    it("should handle multiple chains correctly", () => {
      const marks1 = createTestMarks(3, "low", "alice");
      const marks2 = createTestMarks(3, "low", "bob");
  ...
  describe("test_validate_hash_mismatch", () => {
    it("should detect hash mismatches", () => {
      ...
      const badMark = ProvenanceMark.from({
        res: mark1.res,
        key: mark1.key,
        nextKey: mark0.hash,
        // Wrong! Should be mark1.hash
  ...
  describe("test_validate_multiple_sequences_in_chain", () => {
    it("should handle multiple sequence gaps in a chain", () => {
      const marks = createTestMarks(7, "low", "test");
  ...
  describe("test_validate_precedes_opt", () => {
    it("should validate precedes_opt correctly", () => {
      ...
      // Test valid precedes
      expect(() => marks[0].checkPrecedes(marks[1])).not.toThrow();
      expect(() => marks[1].checkPrecedes(marks[2])).not.toThrow();

      // Test invalid precedes (reverse order)
      expect(() => marks[1].checkPrecedes(marks[0])).toThrow();

      // Test gap
      expect(() => marks[0].checkPrecedes(marks[2])).toThrow();
    });
  });

  describe("test_validate_chain_id_hex", () => {
    it("should correctly encode chain ID as hex", () => {
      const marks = createTestMarks(2, "low", "test");
      const report = validate(marks);
  ...
  describe("test_validate_with_info", () => {
    it("should validate marks with info field", () => {
      const generator = ProvenanceMarkGenerator.fromPassphrase("low", "test");
  ...
  describe("test_validate_sorted_chains", () => {
    it("should sort chains by chain ID", () => {
      const marks1 = createTestMarks(2, "low", "zebra");
      const marks2 = createTestMarks(2, "low", "apple");
      const marks3 = createTestMarks(2, "low", "middle");
  ...
  describe("test_validate_invalid_genesis_key_constructed", () => {
    it("should detect invalid genesis key at non-zero sequence", () => {
      ...
      const badMark = ProvenanceMark.from({
        res: mark1.res,
        key: mark1.chainId,
        // key == chain_id (not allowed at seq > 0)
        nextKey: mark1.hash,
        chainId: mark1.chainId,
        seq: 1,
        // seq 1
